...
describe('JnanaCapture', () => {
    ...
    beforeEach(() => {
        ...
        jnanaCapture = new jnana_capture_1.JnanaCapture(config);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('knowledge capture', () => {
        it('should capture insight', () => {
            const jnana = jnanaCapture.captureInsight('Found a better way to handle async operations', { file: 'utils.ts', line: 42 }, ['async', 'performance']);
        ...
        it('should capture gotcha', () => {
            const jnana = jnanaCapture.captureGotcha('Array.splice modifies the original array', { file: 'array-helpers.ts', line: 15 });
        ...
        it('should capture pattern', () => {
            const jnana = jnanaCapture.capturePattern('Factory pattern for creating different types of parsers');
        ...
        it('should capture question', () => {
            const jnana = jnanaCapture.captureQuestion('How does the event loop handle microtasks vs macrotasks?');
            expect(jnana.category).toBe('question');
        });
        it('should capture generic knowledge', () => {
            const jnana = jnanaCapture.capture('Custom hook for form validation', 'pattern', { file: 'hooks.ts', line: 10 }, ['react', 'hooks', 'forms']);
            expect(jnana.category).toBe('pattern');
    ...
    describe('data import/export', () => {
        ...
        it('should throw error on invalid JSON import', () => {
            expect(() => jnanaCapture.importFromJSON('invalid json')).toThrow('Failed to import jnana from JSON');
        });
    });
    ...
    describe('metadata handling', () => {
        it('should handle custom metadata', () => {
            const jnana = jnanaCapture.capture('Test with metadata', 'insight', undefined, undefined, { source: 'documentation', priority: 'high' });
...
//# sourceMappingURL=jnana-capture.test.js.map