...
describe('SmritiRecall', () => {
  ...
  describe('recent recall', () => {
    beforeEach(async () => {
      ...
      (oldJnana as any).context.timestamp = Date.now() - 10000;
      (newJnana as any).context.timestamp = Date.now() - 5000;
      (newestJnana as any).context.timestamp = Date.now() - 1000;
    ...
    it('should handle empty store', async () => {
      const emptyRecall = new SmritiRecall({ provider: 'memory' }, jnanaCapture);
  ...
  describe('store provider handling', () => {
    it('should handle unsupported providers gracefully', () => {
      ...
      const fallbackRecall = new SmritiRecall({ provider: 'unsupported' as any }, jnanaCapture);
